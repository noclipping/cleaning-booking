...
export default function AdminPage() {
  ...
  const renderServiceDetails = (booking: Booking) => {
    ...
    if (booking.bedrooms || booking.bathrooms) {
      ...
      if (booking.bedrooms) propertyDetails.push(`${booking.bedrooms} bedrooms`);
      if (booking.bathrooms) propertyDetails.push(`${booking.bathrooms} bathrooms`);
      
      details.push(
        <div key="property" className="mb-2">
          <span className="font-medium">Property:</span> {propertyDetails.join(', ')}
        </div>
      );
    }
    ...
    // Appliance services
    const appliances = [];
    if (booking.oven_cleaning) appliances.push(`Oven (${booking.oven_count || 0})`);
    if (booking.microwave_dishwasher_cleaning) appliances.push(`Microwave & Dishwasher (${booking.microwave_dishwasher_count || 0})`);
    if (booking.refrigerator_cleaning) appliances.push(`Refrigerator (${booking.refrigerator_count || 0})`);
    
    if (appliances.length > 0) {
      details.push(
        <div key="appliances" className="mb-2">
          <span className="font-medium">Appliances:</span> {appliances.join(', ')}
        </div>
      );
    }
    ...
    if (booking.wall_cleaning) wallWindowServices.push(`Wall Cleaning (${booking.wall_rooms_count || 0} rooms)`);
    if (booking.interior_window_cleaning) wallWindowServices.push('Interior Windows');
    if (booking.exterior_window_cleaning) {
      wallWindowServices.push(`Exterior Windows (${booking.exterior_windows_count || 0})`);
    }
    
    if (wallWindowServices.length > 0) {
      details.push(
        <div key="wallWindow" className="mb-2">
          <span className="font-medium">Wall & Window Services:</span> {wallWindowServices.join(', ')}
        </div>
      );
    }
    ...
    if (booking.laundry_service) additionalServices.push(`Laundry (${booking.laundry_loads || 0} loads)`);
    if (booking.make_beds) additionalServices.push(`Make Beds (${booking.beds_count || 0} beds)`);
    if (booking.trash_removal) additionalServices.push(`Trash Removal (${booking.trash_bags || 0} bags)`);
    
    if (additionalServices.length > 0) {
      details.push(
        <div key="additional" className="mb-2">
          <span className="font-medium">Additional Services:</span> {additionalServices.join(', ')}
        </div>
      );
    }
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">{getStatusCount('confirmed')}</p>
              </div>
            </div>
          </div>
